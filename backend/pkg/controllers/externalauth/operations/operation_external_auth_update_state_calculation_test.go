			...
			readDesires: []*kubeapplierapi.ReadDesire{
				operationtesting.NewHostedClusterReadDesire(t, &v1beta1.HostedCluster{
					Spec: func() v1beta1.HostedClusterSpec {
						spec := operationtesting.ExternalAuthUpdateMatchingHostedClusterSpec()
						provider := operationtesting.ExternalAuthUpdateMatchingOIDCProvider()
						provider.Name = strings.ToUpper(operationtesting.TestExternalAuthName)
...
func TestHypershiftHostedClusterExternalAuthClaimMappingsSpecMatchesDesired(t *testing.T) {
	...
	newMatchingProvider := func() configv1.OIDCProvider {
		return operationtesting.ExternalAuthUpdateMatchingOIDCProvider()
	}
